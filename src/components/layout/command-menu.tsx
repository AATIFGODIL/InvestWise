...
import { TradingViewTickerTape } from "../shared/trading-view-ticker-tape";
...
export function CommandMenu({ open, onOpenChange }: CommandMenuProps) {
  ...
  const handleDataLoaded = useCallback((data: StockData[]) => {
    setStocks(data);
  }, []);
            ...
            <CommandGroup heading="Stocks">
              {stocks
                ...
                .map((stock) => (
                  <CommandItem
                    key={stock.symbol}
                    onSelect={() => runCommand(() => router.push(`/trade?symbol=${stock.symbol}`))}
